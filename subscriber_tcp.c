...
int main(int argc, char *argv[]) {
    ...
    if (connect(sockfd, (struct sockaddr *)&broker, sizeof(broker)) < 0) {
    ...
    send(sockfd, &msg, sizeof(Mensaje), 0);

    printf("Suscrito al tema %s en broker TCP puerto %d\n", tema, PORT);

    while (1) {
        int n = recv(sockfd, &msg, sizeof(Mensaje), 0);
        if (n > 0) {
